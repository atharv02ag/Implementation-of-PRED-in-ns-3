...
// ── ECN threshold expressed in PACKETS (paper: K = 70 pkts) ──────────────
// MinTh = MaxTh = ECN_K_PKTS → step marking (no gentle ramp).
// RedQueueDisc is configured in PACKETS mode so these values are used
// directly without any bytes conversion.
static const uint32_t MIN_K          = 50;       // ECN marking threshold — min (packets)
static const uint32_t MAX_K          = 200;       // ECN marking threshold — max (packets)
...
/* =========================================================================
 * Build and return a RedQueueDisc helper configured for step-ECN.
 *
 * Key parameters (PACKETS mode):
 *   MinTh = MaxTh = ECN_K_PKTS  → instantaneous step marking at K packets
 *   QW = 1.0                    → no EWMA smoothing (use instantaneous q)
 *   Gentle = false              → no ramp above MaxTh
 *   UseEcn = true, UseHardDrop = false → mark, never drop
 * ========================================================================= */
...
MakeStepEcnHelper()
{
    ...
    tch.SetRootQueueDisc(
        "ns3::RedQueueDisc",
        "MinTh",         DoubleValue(static_cast<double>(MIN_K)),
        "MaxTh",         DoubleValue(static_cast<double>(MAX_K)),
        ...
        "Gentle",        BooleanValue(false));       // true step, no ramp
    return tch;
}
...
main(int argc, char* argv[])
{
     ...
     * =========================================================
     * After each p2p.Install() ns-3 attaches a default pfifo_fast to both
     * devices.  We strip it immediately via StripDefaultQdisc() so that the
     * AQM Install() later finds a clean device with no existing root qdisc.
     * ========================================================= */
     ...
     * ========================================================= */
    TrafficControlHelper tchEcn = MakeStepEcnHelper();
    ...
    fctName << "fct_red_load_" << load << ".csv";
    qName   << "queue_red_load_" << load << ".csv";
    ...
    std::cout
        ...
        << "  RED K (minTh,maxTh): " << MIN_K << " - " << MAX_K << " pkts  [PACKETS mode]\n"
