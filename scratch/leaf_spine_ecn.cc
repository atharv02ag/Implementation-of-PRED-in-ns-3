...
// RedQueueDisc is configured in PACKETS mode so these values are used directly without any bytes conversion.
static constexpr uint32_t ECN_K_PKTS   = 70;     // step-ECN threshold (pkts)
...
MakeStepEcnHelper()
{
    ...
    tch.SetRootQueueDisc(
        "ns3::RedQueueDisc",
        "MinTh",         DoubleValue(static_cast<double>(ECN_K_PKTS)),
        "MaxTh",         DoubleValue(static_cast<double>(ECN_K_PKTS)),
...
main(int argc, char* argv[])
{
    ...
    fctName << "fct_ecn_load_" << load << ".csv";
    qName   << "queue_ecn_load_" << load << ".csv";
    ...
    std::cout
        ...
        << "  ECN K (minTh=maxTh): " << ECN_K_PKTS << " pkts  [PACKETS mode]\n"
