...
static const double TRAFFIC_TIME     = 3;      
static const double SIM_TIME         = 3 + TRAFFIC_TIME;      
...
static const double MIN_K          = (10.0*1500.0)/1000000.0;   // PRED marking threshold — min (in MB)
static const double MAX_K          = 0.500;       // PRED marking threshold — max (in MB)
static const double LAMBDA           = 1.6;      // base λ for PRED (FCS will scale this at runtime)
...
// ─────────────────────────────────────────────────────────
//  Queue monitor at 10 µs granularity
// ─────────────────────────────────────────────────────────
...
int main(int argc, char* argv[])
{
    ...
    Config::SetDefault("ns3::PRedQueueDisc::UseEcn",      BooleanValue(true));
    Config::SetDefault("ns3::PRedQueueDisc::UseHardDrop", BooleanValue(false));
    ...
    // ── Install PRED QueueDisc on switch egress toward receiver ─
    ...
    fctName << "fct_pred_incast_load_" << load << ".csv";
    qName << "queue_pred_incast_load_" << load << ".csv";
    ...
    std::cout << "═══════════════════════════════════════════\n"
              ...
              << "  Sim time     : " << simTime             << " s\n"
              << "  PRED MinTh/MaxTh: " << MIN_K << " / " << MAX_K << " MB\n"
    ...
    std::cout << "Mouse flows completed    : " << completedMouse
              ...
              << " / " << totalElephant << "\n"
              << "Outputs: queue_length_pred.csv  fct_results_pred.csv\n";
